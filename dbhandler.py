...
def get_database_connection():
    if db_type == 'mysql':
        ...
        raise ValueError("Invalid database type. Please set the 'DB_TYPE' environment variable to 'mysql', 'postgres', 'sqlite', or 'replit'.")
...
def get_messages_from_db(phone_number):
    if db_type == 'replit':
        ...
        if db_type == 'sqlite':
            cursor.execute("SELECT `From`, `To`, Role, Message FROM chat WHERE `From`=? OR `To`=? ORDER BY DateTime;",
